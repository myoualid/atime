...
export const OPEN_METEO_MARINE_HOURLY_WAVES =
    'wave_height,wind_wave_height,swell_wave_height,wave_period';
export const OPEN_METEO_MARINE_HOURLY_SEA_AND_WAVES =
    `${OPEN_METEO_MARINE_HOURLY_SEA},${OPEN_METEO_MARINE_HOURLY_WAVES}`;
...
/** @param {unknown} json */
export function parseMarineWaveSeries(json) {
    const hourly = json?.hourly;
    if (!hourly?.time || !Array.isArray(hourly.time)) {
        return {
            time: [],
    ...
    return {
        time: hourly.time,
        waveHeightM: hourly.wave_height ?? [],
        windWaveHeightM: hourly.wind_wave_height ?? [],
        swellWaveHeightM: hourly.swell_wave_height ?? [],
        wavePeriodS: hourly.wave_period ?? [],
    };
}
