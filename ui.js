...
export async function create({core, ops, services}) {
    ...
    const tideUI = createTideSection({ core, services });
    ...
    scrollInner.add(sectionNow, sectionMoon, sectionSeasons, sectionYearMonth, sectionClocks, sectionPhenomena, sectionWeather, sectionTides, sectionSim);
    mainPanel.add(scrollInner);

    const sectionOrder = ['section-now', 'section-moon', 'section-seasons', 'section-year-month', 'section-clocks', 'section-phenomena', 'section-weather', 'section-tides', 'section-sim'];
    const sectionEls = {
    ...
    const footer = DrawUI.div();
    footer.setClass('app-footer');
    footer.dom.setAttribute('role', 'contentinfo');
    footer.dom.innerHTML = `
        <div class="app-footer-inner">
            <div class="app-footer-block app-footer-location">
                <span class="location-icon" aria-hidden="true">📍</span>
                <span class="location-text">${services.LocationService.locationName}</span>
                <span class="location-coords">(${services.LocationService.latitude.toFixed(
                    2,
                )}°, ${services.LocationService.longitude.toFixed(2)}°)</span>
            </div>
    ...
    const footerTime = footer.dom.querySelector('.footer-time');
    const footerTemp = footer.dom.querySelector('.footer-temp');
    const footerWind = footer.dom.querySelector('.footer-wind');
    const footerPhaseTitle = footer.dom.querySelector('.footer-phase-title');
    const footerPhasePct = footer.dom.querySelector('.footer-phase-pct');
    const footerPhaseBarFill = footer.dom.querySelector('.footer-phase-bar-fill');
    ...
    async function refreshFooterWeather() {
        try {
            ...
            if (footerTemp) {
                footerTemp.textContent =
                    t != null && Number.isFinite(t)
                        ? `${t}${u.temperature_2m || '°C'}`
                        : '—';
            }
            if (footerWind) {
                footerWind.textContent =
    ...
    function tick() {
        ...
        if (footerTime) {
            footerTime.textContent = `${pad2(wallNow.getHours())}:${pad2(
                wallNow.getMinutes(),
            )}:${pad2(wallNow.getSeconds())}`;
        }

        const lat = services.LocationService.latitude;
        const lon = services.LocationService.longitude;
        const dn = tools.astro.getDayNightProgress(lat, lon, wallNow);
        if (footerPhaseTitle && footerPhasePct && footerPhaseBarFill) {
            if (dn.phase === 'polar-day' || dn.phase === 'polar-night') {
                footerPhaseTitle.textContent =
                    dn.phase === 'polar-day' ? 'Polar day' : 'Polar night';
                footerPhasePct.textContent = '—';
                footerPhaseBarFill.style.width = '100%';
                footerPhaseBarFill.classList.remove('is-day', 'is-night');
                footerPhaseBarFill.classList.add('is-polar');
            } else {
                footerPhaseTitle.textContent =
                    dn.phase === 'day' ? 'Daylight' : 'Night';
                footerPhasePct.textContent = `${dn.pct.toFixed(1)}%`;
                footerPhaseBarFill.style.width = `${Math.min(
                    100,
                    Math.max(0, dn.pct),
                )}%`;
                footerPhaseBarFill.classList.remove('is-polar');
                footerPhaseBarFill.classList.toggle('is-day', dn.phase === 'day');
                footerPhaseBarFill.classList.toggle(
                    'is-night',
                    dn.phase === 'night',
                );
            }
